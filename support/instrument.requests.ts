  ...
  function requestLogger(httpModule) {
    ...
    if (!httpModule.__instrumented) {
      ...
      httpModule.request = function (options, callback) {
        if ( (options.href || options.host || options.hostname).includes("pndsn") || (options.href || options.host || options.hostname).endsWith("/ack")) {
        ...
        request.on("response", (response) => {
          response.on("end", () => {
            Debugger.logHttpRequest({
              ...
              url: (options.href || options.proto || options.protocol) + "://" + (options.host || options.hostname) + options.path,
              startTime,
