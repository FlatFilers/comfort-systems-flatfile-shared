...
const record_processor_1 = require("./record_processor");
// Mock unpivot processor
jest.mock("./unpivot_processor", () => ({
    createUnpivotedRecords: jest.fn((recordValues, sourceSlug, unpivotGroups) => {
        return [{
                ...recordValues,
                unpivoted: true
            }];
...
const createTestRecord = (values) => Object.entries(values).reduce((acc, [key, value]) => ({
    ...acc,
    [key]: { value }
}), {});
describe("record_processor", () => {
    describe("createStandardRecord", () => {
        ...
        it("should map source fields to target fields", () => {
            ...
            const result = (0, record_processor_1.createStandardRecord)(recordValues, fields);
        ...
        it("should skip undefined values", () => {
            ...
            expect(result === null || result === void 0 ? void 0 : result.target_field1.value).toBe("value1");
            // The empty_field is included in the result, though its value is undefined
            expect(result).toHaveProperty("target_empty");
            expect(result === null || result === void 0 ? void 0 : result.target_empty.value).toBeUndefined();
        ...
        describe("Error Handling", () => {
            it("should handle malformed record values", () => {
                ...
                const result = (0, record_processor_1.createStandardRecord)(malformedValues, fields);
            ...
            it("should handle circular references in record values", () => {
                ...
                const result = (0, record_processor_1.createStandardRecord)(circularValues, fields);
                // The circular reference is preserved in our implementation
                expect(result).toBeDefined();
                expect(result === null || result === void 0 ? void 0 : result.target_field1.value).toBe("test");
            });
        });
        describe("Performance", () => {
            it("should handle large record sets efficiently", () => {
                ...
                const fields = new Map(Object.keys(largeRecordValues).map(key => [key, `target_${key}`]));
                const start = performance.now();
                const result = (0, record_processor_1.createStandardRecord)(largeRecordValues, fields);
            ...
            it("should maintain performance with complex field mappings", () => {
                ...
                const start = performance.now();
                const result = (0, record_processor_1.createStandardRecord)(complexRecordValues, fields);
                const end = performance.now();
                expect(result).toBeDefined();
    ...
    describe("processRecord", () => {
        ...
        it("should process standard record when no unpivot config", () => {
            ...
            const result = (0, record_processor_1.processRecord)(recordValues, "source_sheet", fieldMapping);
        ...
        it("should process unpivot records when unpivot config is provided", () => {
            ...
            const result = (0, record_processor_1.processRecord)(recordValues, "source_sheet", unpivotMapping);
            expect(result.length).toBe(1);
            expect(result[0]).toEqual({
                field1: { value: "value1" },
                unpivot_field: { value: "unpivot_value" },
        ...
        describe("Performance", () => {
            it("should process large batches of records efficiently", () => {
                ...
                const start = performance.now();
                const promises = Array(batchSize).fill(null).map(() => (0, record_processor_1.processRecord)(recordValues, "source_sheet", fieldMapping));
        ...
        describe("Virtual Fields", () => {
            it("should attach virtual fields to each unpivoted record in processRecord", () => {
                ...
                createUnpivotedRecords.mockImplementation((rec, slug, groups, vMap) => {
                ...
                const result = (0, record_processor_1.processRecord)(recordValues, "source_sheet", mapping);
                expect(result.length).toBe(1);
...
//# sourceMappingURL=record_processor.spec.js.map