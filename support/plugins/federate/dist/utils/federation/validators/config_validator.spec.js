...
const config_validator_1 = require("./config_validator");
...
jest.mock("./field_validator", () => ({
    validateFields: (...args) => mockValidators.validateFields(...args),
    validateField: (...args) => mockValidators.validateField(...args)
}));
jest.mock("./merge_validator", () => ({
    validateDedupeConfig: (...args) => mockValidators.validateDedupeConfig(...args)
}));
jest.mock("./unpivot_validator", () => ({
    validateUnpivotConfig: (...args) => mockValidators.validateUnpivotConfig(...args),
    validateUnpivotFields: (...args) => mockValidators.validateUnpivotFields(...args)
}));
jest.mock("./filter_validator", () => ({
    validateFilters: (...args) => mockValidators.validateFilters(...args)
}));
describe("config_validator", () => {
    beforeEach(() => {
        jest.clearAllMocks();
        // Mock validateFields to return field keys and update sourceSheets
        mockValidators.validateFields.mockImplementation((realFields, virtualFields, sheetSlug, sourceSheets, federateConfig) => {
            // Add source sheet slugs from real fields to the provided set
            realFields.forEach((field) => {
                var _a;
                if ((_a = field.federate_config) === null || _a === void 0 ? void 0 : _a.source_sheet_slug) {
            ...
            return new Set(allFields.map((f) => f.key));
        });
    ...
    describe("Basic Validation", () => {
        ...
        it("should throw error for empty sheets", () => {
            ...
            expect(() => (0, config_validator_1.validateConfig)(config)).toThrow("[ConfigValidator] Invalid federation configuration: federated_workbook must contain at least one sheet");
        });
        it("should throw error for duplicate slugs", () => {
            ...
            expect(() => (0, config_validator_1.validateConfig)(config)).toThrow('[ConfigValidator] Duplicate sheet slug found: "duplicate-slug". Sheet slugs must be unique.');
        });
    });
    describe("Field Validation", () => {
        it("should throw error for sheet without fields", () => {
            ...
            expect(() => (0, config_validator_1.validateConfig)(config)).toThrow('[ConfigValidator] Sheet "sheet1" must have at least one field');
        });
        it("should validate field configurations", () => {
            ...
            (0, config_validator_1.validateConfig)(config);
    ...
    describe("Error Handling", () => {
        it("should propagate validation errors", () => {
            ...
            expect(() => (0, config_validator_1.validateConfig)(config)).toThrow("Field validation failed");
        });
        it("should handle unpivot validation errors", () => {
            ...
            expect(() => (0, config_validator_1.validateConfig)(config)).toThrow("Unpivot validation failed");
        });
    });
...
//# sourceMappingURL=config_validator.spec.js.map