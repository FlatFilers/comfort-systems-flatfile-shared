// src/utils/federation/validators/field_validator.ts
...
export function validateFields( // Renamed back from validateAllFields
  ...
  if (federateConfig.debug) logInfo("📦   ↳ Field Validator", `Validating ${realFields.length} real fields and ${virtualFields?.length || 0} virtual fields in sheet "${sheetSlug}"`);
  ...
  const processFieldList = (fieldsToProcess: FederatedProperty[], isVirtual: boolean) => {
    fieldsToProcess.forEach((field) => {
      ...
      if (allFieldKeys.has(field.key)) {
        ...
        /* istanbul ignore next */
        if (federateConfig.debug) logError("📦   ↳ Field Validator", `Field key "${field.key}" (${collisionType}) found in sheet "${sheetSlug}"`);
  ...
  /* istanbul ignore next */
  if (federateConfig.debug) logInfo("📦   ↳ Field Validator", `Validated fields in sheet "${sheetSlug}". Total unique keys: ${allFieldKeys.size}`);
...
function addSourceSheet(field: FederatedProperty, sourceSheets: Set<string>, federateConfig: FederateConfig) {
  if (field.federate_config) {
    ...
    if (sourceSheet) {
      ...
      /* istanbul ignore next */
      if (federateConfig.debug) logInfo("📦   ↳ Field Validator", `Field "${field.key}" references source sheet "${sourceSheet}"`);
    }
    if (sourceSheetSlug) {
      sourceSheets.add(sourceSheetSlug);
      /* istanbul ignore next */
      if (federateConfig.debug) logInfo("📦   ↳ Field Validator", `Field "${field.key}" references source sheet slug "${sourceSheetSlug}"`);
    }
  }
}
...
export function validateField(field: FederatedProperty, federateConfig: FederateConfig): void {
  ...
  if (!field.federate_config) {
    /* istanbul ignore next */
    if (federateConfig.debug) logInfo("📦   ↳ Field Validator", `Field "${field.key}" has no federate_config, skipping validation`);
    return;
  }
  
  /* istanbul ignore next */
  if (federateConfig.debug) logInfo("📦   ↳ Field Validator", `Validating field "${field.key}"`);
  ...
  if (hasSourceFieldKey && !hasSourceSheet && !hasSourceSheetSlug) {
    /* istanbul ignore next */
    if (federateConfig.debug) logError("📦   ↳ Field Validator", `Field "${field.key}" has source_field_key but missing source_sheet_slug`);
  ...
  if (hasSourceSheetSlug && !hasSourceFieldKey) {
    /* istanbul ignore next */
    if (federateConfig.debug) logError("📦   ↳ Field Validator", `Field "${field.key}" has source_sheet_slug but missing source_field_key`);
  ...
  if (hasSourceFieldKey && !hasSourceSheet && !hasSourceSheetSlug) {
    /* istanbul ignore next */
    if (federateConfig.debug) logError("📦   ↳ Field Validator", `Field "${field.key}" has source_field_key but is missing both source_sheet and source_sheet_slug`);
  ...
  if (hasSourceSheet && hasSourceFieldKey && !federateConfig.allow_undeclared_source_fields) {
    ...
    if (!sourceSheet) {
      /* istanbul ignore next */
      if (federateConfig.debug) logError("📦   ↳ Field Validator", `Field "${field.key}" has undefined source_sheet`);
    ...
    if (!fieldExists) {
      /* istanbul ignore next */
      if (federateConfig.debug) logError("📦   ↳ Field Validator", `Field "${sourceFieldKey}" not found in source sheet "${sourceSheet.slug}"`);
      throw new Error(`[FieldValidator] Field "${sourceFieldKey}" not found in source sheet "${sourceSheet.slug}"`);
    }
    
    /* istanbul ignore next */
    if (federateConfig.debug) logInfo("📦   ↳ Field Validator", `Field "${field.key}" validated successfully, source field "${sourceFieldKey}" exists in source sheet "${sourceSheet.slug}"`);
  }
  
  /* istanbul ignore next */
  if (federateConfig.debug) logInfo("📦   ↳ Field Validator", `Field "${field.key}" validated successfully`);
}