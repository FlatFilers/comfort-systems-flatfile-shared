...
export function validateConfig(config: FederateConfig): Set<string> {
  ...
  config.federated_workbook.sheets.forEach((sheet, index) => {
    ...
    if (config.debug) logInfo("📦  ↳ Config Validator", `Validating fields for sheet "${sheet.slug}"`);
    const fieldKeys = validateFields(sheet.fields, sheet.virtualFields, sheet.slug!, sourceSheets, config);
    
    // Validate merge configuration
    /* istanbul ignore next */
    if (config.debug) logInfo("📦  ↳ Config Validator", `Validating merge configuration for sheet "${sheet.slug}"`);
    ...
    /* istanbul ignore next */
    if (config.debug) logInfo("📦  ↳ Config Validator", `Validating unpivot configuration for sheet "${sheet.slug}"`);
    validateUnpivotConfig(sheet, config);
    
    // Validate filters
    /* istanbul ignore next */
    if (config.debug) logInfo("📦  ↳ Config Validator", `Validating filters for sheet "${sheet.slug}"`);
