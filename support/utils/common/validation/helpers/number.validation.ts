...
export class NumberValidator {
  ...
  public static isPositive(
    record: Record<string, any>,
    field: string,
    options?: { addError?: boolean; validateOnEmpty?: boolean; errorMsg?: string; includeZero?: boolean },
  ) {
    let value: string | undefined;
    let isRecordWithoutLinks = false;
    try {
    ...
    if (value || options?.validateOnEmpty) {
      if (!valid && options?.addError && !isRecordWithoutLinks) {
        var errorMsg =
          options?.errorMsg ||
          (options?.includeZero ? "Value must be greater than or equal to 0" : "Value must be greater than 0");
        record.addError(field, errorMsg);
      }
    }
  ...
  public static isNegative(
    ...
    if (value || options?.validateOnEmpty) {
      if (!valid && options?.addError && !isRecordWithoutLinks) {
        var errorMsg =
          options?.errorMsg ||
          (options?.includeZero ? "Value must be less than or equal to 0" : "Value must be less than 0");
        record.addError(field, errorMsg);
      }
    }
    ...
    switch (validationType) {
      ...
      case NumberValidationType.RANGE:
        ...
        if (Array.isArray(validationArgs) && validationArgs.length === 2) {
          minValue = typeof validationArgs[0] === "number" ? validationArgs[0] : 0;
          maxValue = typeof validationArgs[1] === "number" ? validationArgs[1] : 0;
        }
        isValid = this.range(record, field, minValue, maxValue, options);
        break;
      case NumberValidationType.IS_INTEGER:
        isValid = this.isInteger(record, field, options);
        break;
      case NumberValidationType.IS_POSITIVE:
        isValid = this.isPositive(record, field, options);
        break;
      case NumberValidationType.IS_NEGATIVE:
        isValid = this.isNegative(record, field, options);
        break;
      case NumberValidationType.IS_MULTIPLE_OF:
        isValid = this.isMultipleOf(record, field, validationArgs, options);
        break;
      case NumberValidationType.HAS_PRECISION:
        isValid = this.hasPrecision(record, field, validationArgs, options);
        break;
      case NumberValidationType.VALIDATE:
      default:
        isValid = this.validate(record, field, options);
        break;
    }
